...
class SMItracker(BaseEyeTracker):
	...
	def __init__(self, display, ip='169.254.17.229', sendport=4444,
		...
		**args):
		...
		logfile	-- logfile name (string value); note that this is the
				   ...
				   (default = LOGFILE)
	        self.outputfile is the .idf file - see line 170
		"""
		...
		# self.outputfile is the .idf. The path it sets is the path it will end up at on the laptop 
		self.outputfile = "C:/Users/iView x/Desktop/mjgreen/this_is_the_idf" + logfile[30::] + ".idf"
		self.description = "experiment" # TODO: EXPERIMENT NAME
		self.participant = logfile[30::]
		...
		# connect to iViewX (notice how we redefine res two lines below)
		res = iViewXAPI.iV_Connect(c_char_p(ip), c_int(sendport), c_char_p(ip), c_int(receiveport))
		res = iViewXAPI.iV_Connect(c_char_p('169.254.17.229'), c_int(4444), c_char_p('169.254.17.228'), c_int(5555))
		if res == 1:
